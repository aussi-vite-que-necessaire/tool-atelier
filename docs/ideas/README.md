...
- [Fiabiliser l'étape `deploy` (pull + run sur le lab)](2026-05-29-fiabiliser-deploy-lab.md) — À explorer — Retry/backoff sur `docker pull` + éventuelle sérialisation des deploys concurrents, après que le job `deploy` a planté pour les 3 projets (builds CI OK) sur hoquet pull GHCR / RAM lab.
