...
# l'atelier ajoute quelques skills dédiées (/noter-idee, /travailler-infra, /apercu, lab-ssh).
set -uo pipefail
...
base="Dis ce que tu veux faire — le dev de feature passe par superpowers ; skills atelier : \`/noter-idee\`, \`/travailler-infra\`, \`/apercu\` (l'œil sur le front), \`lab-ssh\`. On code sur une branche de session, PR ; push de branche = preview, PR mergée = intégration (sur *.preview), prod = promotion explicite."
