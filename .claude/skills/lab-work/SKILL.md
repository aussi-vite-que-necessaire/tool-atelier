...
1. Si le projet n'est pas donné, scanne `projects/*/lab.json` et demande lequel (nom + description courte).
2. Vérifie que `projects/<projet>/` existe (dossier avec un `Dockerfile`).
...
4. Annonce : « Session focalisée sur **<projet>** (branche `work/...`). Je travaille dans `projects/<projet>/`. »
5. **Travaille uniquement dans `projects/<projet>/`.** Lis son `CLAUDE.md` pour le contexte du projet.
