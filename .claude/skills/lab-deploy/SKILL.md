...
3. Commit les changements, puis `git push -u origin <branche>`.
   → déclenche une **preview** : `https://<projet>-<branche>.lab.avqn.ch`.
...
5. **Prod** : ouvre une PR (`gh pr create --fill`) ; après relecture, **merge** la PR → déploie `https://<projet>.lab.avqn.ch`.
