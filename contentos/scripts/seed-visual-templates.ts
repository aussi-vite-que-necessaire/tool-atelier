...
// Résout l'argument en userId : accepte un userId direct OU un email
// (plus pratique : on connaît son email après signup).
...
async function main(): Promise<void> {
  ...
  if (!arg) {
    console.error('Usage: tsx --env-file=.env scripts/seed-visual-templates.ts <userId|email>');
    process.exit(1);
  }
  ...
  const { created, skipped } = await seedVisualTemplates(userId);
  console.log(`\nDone. user=${userId} created=${created} skipped=${skipped}`);
  process.exit(0);
}
