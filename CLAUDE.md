...
- **Push de branche → preview par-branche** : `https://app-<branche>.preview.contentos.ch` (la suite entière, isolée, base dédiée clonée-scrubbée depuis la prod ; détruite à la suppression de la branche).
- **Merge de PR → `main` = palier d'intégration** : `https://app.preview.contentos.ch`, base clonée intégralement depuis la prod (jamais détruite), chaque déploiement gardé par un contrôle santé (web `/healthz` + liveness du worker) avec **rollback automatique** vers le dernier artefact sain si le contrôle échoue. Un déploiement d'intégration vert pose le tag `:integration` sur son artefact — donc `:integration` désigne toujours une image passée par l'intégration de bout en bout. C'est le palier stable où l'on valide avant prod.
- **Promotion explicite → prod** : `https://contentos.ch` (+ `www.contentos.ch`), via le workflow `promote` (`workflow_dispatch`) qui **re-tague l'artefact `:integration` en `:prod` sans rebuild** (on promeut l'image exacte validée à l'intégration). Prod n'est jamais un envoi auto au merge.
