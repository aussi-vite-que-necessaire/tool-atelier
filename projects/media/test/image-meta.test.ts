...
import { parseImageDimensions, extensionForMime, mimeForFormat } from "../src/lib/image-meta";

function makePng(width: number, height: number): Uint8Array {
  const b = new Uint8Array(24);
  // Signature PNG
  b.set([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a], 0);
  // Longueur IHDR (13) + "IHDR" (offsets 8..15), non lus par le parser
  b.set([0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44, 0x52], 8);
  // width (offset 16) et height (offset 20), big-endian
  b[16] = (width >>> 24) & 0xff;
  b[17] = (width >>> 16) & 0xff;
  b[18] = (width >>> 8) & 0xff;
  b[19] = width & 0xff;
...
function makeJpeg(width: number, height: number): Uint8Array {
  // SOI + SOF0 portant les dimensions
