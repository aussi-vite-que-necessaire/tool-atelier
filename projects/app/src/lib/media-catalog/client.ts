...
function base(): string {
  if (!env.MEDIA_ENGINE_URL || !env.MEDIA_ENGINE_SERVICE_KEY)
    throw new Error('MEDIA_ENGINE_URL/KEY manquants');
  return env.MEDIA_ENGINE_URL.replace(/\/$/, '');
}
function authHeaders() {
  return { Authorization: `Bearer ${env.MEDIA_ENGINE_SERVICE_KEY}` };
}
...
export async function listMedia(
  ...
  const res = await fetch(`${base()}/v1/media?${qs}`, {
    headers: authHeaders(),
    cache: 'no-store',
  });
...
export async function getMedia(userId: string, id: string): Promise<MediaItem | null> {
  ...
  const res = await fetch(`${base()}/v1/media/${encodeURIComponent(id)}?${qs}`, {
    headers: authHeaders(),
    cache: 'no-store',
  });
